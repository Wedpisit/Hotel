<template>
  <div>
    ...
    <div class="hero-wrapper">
      <img src="@/assets/hoteltwo.png" width="230"  height="200" class="logo" style="float: left" />
      <h1>ระบบจองห้องพักในโรงแรม </h1>
      <h3>HOTEL BOOKING SYSTEM</h3>
      <p><i class="fas fa-hotel"></i>&nbsp;WECHLEE HOTEL</p>
    </div>
    <div class="clearfix"></div>
    <div class="blog-header">
      <div>
        <form class="form-inline form-search">
          <span class="font2"><strong> <i class="fas fa-search"></i>&nbsp;ค้นหาห้องพักจากทั้งหมด : </strong> {{ results.length }} ห้อง</span>
          &nbsp;
    ...
    <transition-group name="fade">
      <div v-for="blog in blogs" v-bind:key="blog.id" class="blog-list">
        ...
        <h3>{{ blog.title }}</h3>
        <div class="blog-info">
          <p><strong>สำหรับนอน:</strong> {{ blog.category }}</p>
          <p><strong>บริการ:</strong> {{ blog.status }}</p>
          <p><strong>ราคา/คืน:</strong> {{ blog.price}}</p>
          ...
          <p>
            ...
            >
              <i class="far fa-eye"></i> ดูข้อมูลห้องพัก
            </button>
          </p>
        </div>
        <div class="clearfix"></div>
    ...
    <div v-if="blogs.length === 0 && loading === false" class="emptyblog">
      <center>*** ไม่มีข้อมูลห้องพัก ***</center>
    </div>
    <div id="blog-list-bottom">
      ...
      >
        <span class="font3">โหลดข้อมูลห้องพักครบแล้ว</span>
      </div>
    </div>
  </div>
</template>
